...
class jpHarmonicNode(OpenMayaMPx.MPxDeformerNode):
	...
	def deform(self,dataBlock,geomIter,matrix,multiIndex):
		...
		harmVec = OpenMaya.MVector(float(-1 * harmX), float(harmY), float(harmZ))
...
def nodeInitializer():
	...
	#wavelength channel
	jpHarmonicNode.wavelength = nAttr.create( "wavelength", "wle", OpenMaya.MFnNumericData.kFloat, 1.0 )
	nAttr.setKeyable(True)
